<script lang="ts">
  ...
  let jobs: any[] = $state([]), banks: ScanRange[] = $state([]), kind = $state('scan'), range = $state('FM Broadcast'), delay = $state(1), duration = $state(60), notice = $state('');
  ...
  async function create(){ try { const payload=kind==='scan'?{range_name:range,duration_ms:Math.max(1,duration)*1000}:{duration_ms:Math.max(1,duration)*1000}; await Api.createJob({name:kind==='scan'?`${range} scan`:'IQ recording',kind,payload,next_run_ms:Date.now()+Math.max(0,delay)*1000}); await load(); notice='Scheduled.' }catch(e){notice=String(e)} }
...
<div class="card form"><label>Job <select bind:value={kind}><option value="scan">Scan</option><option value="recording">IQ recording</option></select></label>{#if kind === 'scan'}<label>Range <select bind:value={range}>{#each banks as b}<option value={b.name}>{b.name}</option>{/each}</select></label>{/if}<label>Start in seconds <input type="number" min="0" bind:value={delay}/></label><label>Duration seconds <input type="number" min="1" max="3600" bind:value={duration}/></label><button onclick={create} disabled={kind === 'scan' && !range}>Schedule {kind}</button></div>
