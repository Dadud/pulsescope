...
<div class="page">
  ...
  <div class="decoder-grid">
    {#each visibleDecoders as decoder (decoder.id)}
      <section class="decoder-card" class:verified={decoder.available}>
        <div class="title"><h2>{decoder.name}</h2><span class:installed={decoder.available || decoder.status === 'installed' || decoder.status === 'fixture_verified'} class="badge">{decoder.available || decoder.status === 'fixture_verified' ? 'Fixture verified' : decoder.status === 'installed' ? 'Installed' : 'Beta'}</span></div>
