...
//!
//! Feeds demodulated 48kHz mono S16LE audio continuously over stdin,
...
pub fn find_dsd_fme() -> Option<PathBuf> {
    ...
    // System locations only. Managed copies are resolved by depmanager from
    // the application's data directory; no developer/user directory is used.
    let candidates: Vec<PathBuf> = if cfg!(windows) {
        vec![PathBuf::from("dsd-fme.exe")]
    } else {
        vec![
...
pub fn decode_digital_voice(samples: &[f32], mode: &str) -> DsdResult {
    ...
    if let Some(mut input) = child.stdin.take() {
        ...
        if let Err(e) = input.write_all(&pcm) {
            let _ = child.kill();
            return DsdResult {
                available: false,
                decoder_path: Some(exe.display().to_string()),
                error_message: Some(format!("dsd-fme stdin failed: {e}")),
                ..Default::default()
            };
        }
    }
