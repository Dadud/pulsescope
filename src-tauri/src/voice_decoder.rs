...
pub fn find_dsd_fme() -> Option<PathBuf> {
    ...
    let candidates: Vec<PathBuf> = if cfg!(windows) {
        vec![
            PathBuf::from(r"C:\Users\Dadud\pulsescope\decoders\dsd-fme\dsd-fme-portable\dsd-fme\dsd-fme.exe"),
    ...
    if let Ok(sdr_root) = std::env::var("SOAPY_SDR_ROOT") {
        let p = PathBuf::from(&sdr_root).join("bin").join(if cfg!(windows) { "dsd-fme.exe" } else { "dsd-fme" });
    ...
    // PATH search
    which::which(if cfg!(windows) { "dsd-fme.exe" } else { "dsd-fme" }).ok()
}
...
pub fn decode_digital_voice(samples: &[f32], mode: &str) -> DsdResult {
    let exe = match find_dsd_fme() {
        Some(p) => p,
        None => return DsdResult {
            available: false,
            error_message: Some("dsd-fme not found. Install from https://github.com/lwvmobile/dsd-fme/releases".into()),
            ..Default::default()
        },
    };
    ...
    for line in combined.lines() {
        ...
        if trimmed.contains("Call:") || trimmed.contains("SRC:") {
            if let Some(call) = trimmed.split("Call:").nth(1).or_else(|| trimmed.split("SRC:").nth(1)) {
                let call = call.split_whitespace().next().unwrap_or("").trim_end_matches(',').to_string();
        ...
        if trimmed.contains("TG:") || trimmed.contains("TGID:") {
            if let Some(tg) = trimmed.split("TG:").nth(1).or_else(|| trimmed.split("TGID:").nth(1)) {
                let tg = tg.split_whitespace().next().unwrap_or("").trim_end_matches(',').to_string();
        ...
        if trimmed.contains("NAC:") {
            if let Some(nac) = trimmed.split("NAC:").nth(1) {
                let nac = nac.split_whitespace().next().unwrap_or("").trim_end_matches(',').to_string();
        ...
        if trimmed.contains("audio errors") {
            if let Some(num) = trimmed.split("errors").next().and_then(|s| s.split_whitespace().last()) {
