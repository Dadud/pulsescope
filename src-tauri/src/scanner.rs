...
pub struct ScannerRuntimeState {
    ...
    /// `band` = constrain pan/tuning to active scan range; `full` = device tuning range.
            ...
            match cmd {
                ScannerCommand::Start { range, cycle } => {
                    ...
                    let vfos = starter_vfos(&range, device.status().center_freq_hz, cfg.max_vfos);
                    {
                        ...
                        runtime.spectrum_view_mode = "band".into();
                ...
                ScannerCommand::SetSpectrumViewMode { mode } => {
                    ...
                    if normalized == "band" {
                        if let Some(range) = active_range.as_ref() {
                            let status = device.status();
                            let (device_min, device_max) = device.tuning_range_hz();
                            ...
                            if let Err(error) = device.set_frequency(center) {
                                tracing::warn!(%error, center_hz = center, "failed to recenter for band mode");
                            }
                        }
                    }
                }
                ScannerCommand::PanCenter { center_hz } => {
                    ...
                    if let Err(error) = device.set_frequency(clamped) {
                        tracing::warn!(%error, center_hz = clamped, "pan center failed");
                    } else {
                        capture_ring.clear();
                        audio_ring.clear();
                        snapshot_ring.clear();
