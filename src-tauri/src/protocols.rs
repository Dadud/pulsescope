...
pub fn slices() -> Vec<ProtocolSlice> {
    vec![
        ...
        slice_verified("goes-satdump", "GOES LRIT/HRIT product identification", "GOES-East/West downlink selected from current published plan", 1_694_100_000, 1_500_000, 2_400_000, "CCSDS ASM 0x1ACFFC1D", "BPSK", "Not claimed for native CADU identification; SatDump concatenated coding remains sidecar-only", "CRC-32 over post-ASM product body", "goes.product.v1: satellite, product, channel, timestamp, file_path, valid", "fixtures/recorded-iq/goes-lrit-info.iq.json", "Validated products/gallery; never claim reception from configured output paths", "native BPSK CADU -> goes.product.v1; SatDump CLI IQ pipeline when a signed manifest is installed"),
        slice_verified("radiosondes", "Radiosondes", "Model/local launch dependent, commonly 400.15-406 MHz", 403_000_000, 20_000, 192_000, "0xAA preamble and 0x10B6 sync", "GFSK 4800 baud", "Not claimed for native RS41-class frames; live Vaisala RS FEC remains sidecar-only", "CRC-16-CCITT", "radiosonde.telemetry.v1: model, serial, frame, lat, lon, altitude_m, temperature_c, checksum_valid", "fixtures/recorded-iq/radiosonde-rs41-pulse.iq.json", "Telemetry/map only for checksum-valid frames", "native GFSK CRC-16 radiosonde.telemetry.v1; rs41mod stdout adapter when installed"),
...
mod tests {
    ...
    fn every_priority_slice_has_a_complete_preimplementation_contract() {
        for s in slices() {
            ...
            if matches!(
                s.id,
                "ble-advertising" | "lora-mesh" | "lorawan" | "goes-satdump" | "radiosondes"
            ) {
                assert!(s.available);
            } else {
                assert!(!s.available);
