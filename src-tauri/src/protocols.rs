...
pub fn slices() -> Vec<ProtocolSlice> {
    vec![
        slice("ble-advertising", "BLE advertising", "Advertising channels 37/38/39: 2402/2426/2480 MHz", 2_402_000_000, 2_000_000, 4_000_000, "8-bit preamble + advertising access address 0x8E89BED6", "1 Msym/s GFSK, BT=0.5", "None", "BLE CRC-24 after channel whitening", "ble.advertisement.v1: address, address_type, rssi_dbm, ad_structures, raw_pdu", "fixtures/iq/ble_advertising.cf32 (not yet cleared/recorded)", "Device table and detail view; no synthetic devices", "native CF32 channelizer -> GFSK/whitening parser"),
        slice("lorawan", "LoRa / LoRaWAN", "Regional plan selected explicitly (EU868 default plan document only)", 868_100_000, 125_000, 1_000_000, "LoRa up-chirp preamble, sync word, SFD", "CSS SF7-SF12", "Hamming(4,n), diagonal interleaving", "PHY CRC; LoRaWAN MIC (AES-CMAC, keys supplied by user)", "lorawan.frame.v1: region, frequency, sf, bandwidth, devaddr, fctrl, fcnt, fport, payload, mic_valid", "fixtures/iq/lorawan_eu868.cf32 (not yet cleared/recorded)", "Regional packet table; encrypted payload remains opaque without keys", "native CF32 LoRa PHY -> LoRaWAN parser"),
