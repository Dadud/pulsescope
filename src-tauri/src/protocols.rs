...
pub fn slices() -> Vec<ProtocolSlice> {
    vec![
        ...
        slice_verified("lora-mesh", "LoRa mesh and Modbus", "Regional plan selected explicitly (US915/EU868 documented plans only)", 915_000_000, 125_000, 1_000_000, "LoRa up-chirp preamble and SFD", "CSS SF7 125 kHz", "Implicit-header CSS fixture; Hamming FEC not claimed", "PHY recovery plus Modbus CRC-16; mesh/LoRaWAN MIC never decrypted", "lora.mesh.v1: protocol, address, message_type, content, encryption", "fixtures/recorded-iq/lora-meshtastic-hello.iq.json", "Packet table with MeshCore/Meshtastic/Reticulum/Modbus families; encrypted bodies stay opaque", "native CSS PHY -> mesh/Modbus/LoRaWAN identifier"),
        slice("lorawan", "LoRaWAN identification", "Regional plan selected explicitly (EU868 default plan document only)", 868_100_000, 125_000, 1_000_000, "LoRa up-chirp preamble, sync word, SFD", "CSS SF7-SF12", "Hamming(4,n), diagonal interleaving", "PHY CRC; LoRaWAN MIC identified, never decrypted", "lorawan.frame.v1: region, frequency, sf, bandwidth, devaddr, fctrl, fcnt, fport, payload, mic_valid", "fixtures/iq/lorawan_eu868.cf32 (not yet cleared/recorded)", "Regional packet table; encrypted payload remains opaque", "native CF32 LoRa PHY -> LoRaWAN identifier"),
...
mod tests {
    ...
    fn every_priority_slice_has_a_complete_preimplementation_contract() {
        for s in slices() {
            ...
            if matches!(s.id, "ble-advertising" | "lora-mesh") {
                assert!(s.available);
            } else {
                assert!(!s.available);
            }
