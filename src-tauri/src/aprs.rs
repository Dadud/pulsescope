...
impl AprsDecoder {
    ...
    fn goertzel_step(&mut self, x: f32, freq: f32) -> f32 {
        ...
        if freq == MARK_HZ {
            self.mark_phase += phase_inc;
        } else {
            self.space_phase += phase_inc;
        }
        let phase = if freq == MARK_HZ {
            self.mark_phase
        } else {
            self.space_phase
        };
...
mod tests {
    ...
    fn goertzel_detects_aprs_tones() {
        ...
        let mark: Vec<f32> = (0..8000)
            .map(|i| (TAU * MARK_HZ * i as f32 / sample_rate).sin() * 0.5)
            .collect();
        let mark_e = goertzel(&mark, MARK_HZ, sample_rate);
        let space_e = goertzel(&mark, SPACE_HZ, sample_rate);
        assert!(
            mark_e > space_e * 5.0,
            "mark energy ({mark_e}) should dominate at 1200Hz"
        );
        // 1 second of pure 2200 Hz
        let space: Vec<f32> = (0..8000)
            .map(|i| (TAU * SPACE_HZ * i as f32 / sample_rate).sin() * 0.5)
            .collect();
        let mark_e = goertzel(&space, MARK_HZ, sample_rate);
        let space_e = goertzel(&space, SPACE_HZ, sample_rate);
        assert!(
            space_e > mark_e * 5.0,
            "space energy ({space_e}) should dominate at 2200Hz"
        );
    }
    ...
    fn parse_ax25_bits_round_trip_known_payload() {
        ...
        assert_eq!(
            bits_extracted, direct_info,
            "info bytes via bits path should match direct path"
        );
        assert_eq!(
            frames.len(),
            1,
            "expected 1 frame, got {} (digis={:?})",
            frames.len(),
            frames.first().map(|f| &f.digipeaters)
        );
    ...
    fn afsk_demod_extracts_bits() {
        ...
        assert!(
            mark_dominant > 0 && space_dominant > 0,
            "expected alternating tone dominance, got mark={} space={}",
            mark_dominant,
            space_dominant
        );
    }
