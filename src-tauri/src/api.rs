...
pub async fn serve(cfg: ServeConfig, state: Arc<AppState>) -> anyhow::Result<()> {
    ...
    let api = Router::new()
        ...
        .merge(trunking::router())
        .merge(decoders::router())
...
async fn auth_gate(
    ...
    let header = req
        ...
        .filter(|v| !v.is_empty());
    // Reject duplicate token parameters rather than accepting whichever one
    // happened to be first. Percent decoding is required for browser WS URLs.
    let query_tokens = req
        ...
        .map(|(_, value)| percent_encoding::percent_decode_str(value).decode_utf8())
        .collect::<Vec<_>>();
    let query = match query_tokens.as_slice() {
