...
struct TrafficGuard {
    ...
    window: Arc<std::sync::Mutex<(std::time::Instant, u32)>>,
}
...
pub struct ServeConfig {
    ...
    /// Exact browser origins allowed to call the API. Empty means the desktop
    /// loopback origins only; `*` is intentionally unsupported.
...
pub async fn serve(cfg: ServeConfig, state: Arc<AppState>) -> anyhow::Result<()> {
    ...
    let security = SecurityState::new(auth_token);
    let api = Router::new()
        // ── health / settings ────────────────────────────────────────────
        ...
        .route("/auth/ws-ticket", post(crate::security::ws_ticket))
        .route("/auth/token/rotate", post(crate::security::rotate_token))
        .route("/auth/token/revoke", post(crate::security::revoke_tokens))
    ...
    let origin_values: Vec<HeaderValue> = origins.iter().filter_map(|v| v.parse().ok()).collect();
    let traffic = TrafficGuard {
        concurrent: Arc::new(tokio::sync::Semaphore::new(64)),
        window: Arc::new(std::sync::Mutex::new((std::time::Instant::now(), 0))),
    };
    top = top
        .layer(axum::extract::DefaultBodyLimit::max(1024 * 1024))
        .layer(TimeoutLayer::new(std::time::Duration::from_secs(30)))
        .layer(axum::middleware::from_fn_with_state(traffic, traffic_gate))
...
async fn traffic_gate(
    axum::extract::State(guard): axum::extract::State<TrafficGuard>,
    ...
    {
        let mut window = guard.window.lock().expect("traffic lock");
        if window.0.elapsed() >= std::time::Duration::from_secs(1) {
            *window = (std::time::Instant::now(), 0);
        }
        // Lookup endpoints have a deliberately smaller shared budget because
        // they may cause paid/privacy-sensitive external requests.
        let limit = if req.uri().path().contains("lookup") {
            10
        } else {
            200
        };
...
async fn no_store(
    ...
    response.headers_mut().insert(header::CONTENT_SECURITY_POLICY, HeaderValue::from_static("default-src 'self'; connect-src 'self' ws: wss:; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self'"));
    response
}
...
async fn auth_gate(
    ...
    // Only the WS endpoint accepts a URL credential, and that credential is a
    // 30-second, single-use ticket rather than the long-lived API token.
    let ticket_ok = path.ends_with("/events")
        && req
            .uri()
            .query()
            .and_then(|q| q.split('&').find_map(|kv| kv.strip_prefix("ticket=")))
            .is_some_and(|ticket| security.consume_ticket(ticket));
    if security.authenticate(bearer) || ticket_ok {
        tracing::info!(target: "security_audit", method = %req.method(), path = %path, action = "request.authorize", outcome = "success");
        Ok(next.run(req).await)
    } else {
        tracing::warn!(target: "security_audit", method = %req.method(), path = %path, action = "request.authorize", outcome = "denied");
...
async fn vfo_frequency(
    ...
    Json(json!({"ok":true,"id":id,"frequency_hz":req.frequency_hz}))
}
#[derive(Deserialize)]
struct VfoModeReq {
    mode: String,
...
async fn transcription_start(State(s): State<ApiState>) -> impl IntoResponse {
    ...
    let _ = c.save(&s.0.data_dir);
    Json(
        json!({"ok":true,"running":true,"engine":c.transcription.engine,"model":c.transcription.model}),
    )
}
