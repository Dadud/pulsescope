...
pub enum DeviceLifecycle { Detected, Probing, Configuring, Streaming, Degraded, Recovering, #[default] Disconnected }
...
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct DeviceStatus { pub connected: bool, pub lifecycle: DeviceLifecycle, pub driver: String, pub label: String, pub sample_rate: u32, pub center_freq_hz: u64, pub ppm_correction: f32, pub gain: String, pub bias_tee_on: bool, pub saturation: bool, pub stream: StreamCountersSnapshot }
...
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
pub struct DeviceSetting { pub key: String, pub name: String, pub value: String, pub kind: String, pub min: f64, pub max: f64, pub step: f64, pub options: Vec<String> }
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
pub struct NumericRange { pub minimum: f64, pub maximum: f64, pub step: f64 }
#[derive(Clone, Debug, Default, Serialize, Deserialize)]
...
pub struct DeviceCapabilities {
    ...
    pub rf_ranges_hz: Vec<NumericRange>, pub sample_rate_ranges_hz: Vec<NumericRange>,
    pub supported_sample_rates_hz: Vec<u32>, pub bandwidth_ranges_hz: Vec<NumericRange>,
    pub sample_formats: Vec<String>, pub stream_mtu: usize,
    pub total_bandwidth_hz: u32, pub usable_bandwidth_hz: u32,
    pub tuner_count: u32, pub full_duplex: bool,
    pub gain_stages: Vec<GainStage>, pub settings: Vec<DeviceSetting>, pub antennas: Vec<String>, pub antenna: String,
...
mod soapy {
    ...
    pub struct Hardware { device: *mut s::SoapySDRDevice, stream: *mut s::SoapySDRStream, stream_mtu: usize }
    ...
    unsafe fn numeric_ranges(p: *mut s::SoapySDRRange, length: usize) -> Vec<NumericRange> {
        ...
        let values = std::slice::from_raw_parts(p, length).iter()
            .map(|range| NumericRange { minimum: range.minimum, maximum: range.maximum, step: range.step })
            .collect();
    ...
    impl Hardware {
        ...
        pub fn apply_safe_defaults(&mut self) -> anyhow::Result<()> { unsafe {
            let dir=s::SOAPY_SDR_RX as i32;
            if s::SoapySDRDevice_hasGainMode(self.device,dir,0) { check("enable AGC",s::SoapySDRDevice_setGainMode(self.device,dir,0,true))?; }
            if s::SoapySDRDevice_hasDCOffsetMode(self.device,dir,0) { check("enable DC offset auto",s::SoapySDRDevice_setDCOffsetMode(self.device,dir,0,true))?; }
            if s::SoapySDRDevice_hasIQBalanceMode(self.device,dir,0) { check("enable IQ balance auto",s::SoapySDRDevice_setIQBalanceMode(self.device,dir,0,true))?; }
            // SDRplay exposes these as Soapy vendor settings. Apply only when
            // present; other hardware retains its own driver defaults.
            for (key, value) in [("iqcorr_ctrl", "true"), ("agc_setpoint", "-30"), ("rfgain_sel", "1")] {
                let key_c=CString::new(key).unwrap();
                if !s::SoapySDRDevice_readSetting(self.device,key_c.as_ptr()).is_null() {
                    let value_c=CString::new(value).unwrap();
                    check("apply receiver default",s::SoapySDRDevice_writeSetting(self.device,key_c.as_ptr(),value_c.as_ptr()))?;
                }
            }
            Ok(())
        }}
        pub fn capabilities(&self) -> DeviceCapabilities { unsafe {
            let mut c=DeviceCapabilities{contract_version:1,connected:true,stream_mtu:self.stream_mtu,sample_formats:vec!["CF32".into()],tuner_count:1,..Default::default()}; let dir=s::SOAPY_SDR_RX as i32;
            c.tuner_count=s::SoapySDRDevice_getNumChannels(self.device,dir) as u32;
            c.full_duplex=c.tuner_count>0&&s::SoapySDRDevice_getFullDuplex(self.device,dir,0);
            let mut n=0usize; c.rf_ranges_hz=numeric_ranges(s::SoapySDRDevice_getFrequencyRange(self.device,dir,0,&mut n),n);
            let mut n=0usize; c.sample_rate_ranges_hz=numeric_ranges(s::SoapySDRDevice_getSampleRateRange(self.device,dir,0,&mut n),n);
            c.supported_sample_rates_hz=c.sample_rate_ranges_hz.iter().filter(|range| range.minimum==range.maximum&&range.minimum>0.0&&range.minimum<=u32::MAX as f64).map(|range|range.minimum as u32).collect();
            let mut n=0usize; c.bandwidth_ranges_hz=numeric_ranges(s::SoapySDRDevice_getBandwidthRange(self.device,dir,0,&mut n),n);
            let mut n=0usize; let formats=s::SoapySDRDevice_getStreamFormats(self.device,dir,0,&mut n); if !formats.is_null(){c.sample_formats=(0..n).map(|i|CStr::from_ptr(*formats.add(i)).to_string_lossy().into_owned()).collect();s::SoapySDRStrings_clear(&mut (formats as *mut *mut c_char),n);}
            let current_rate=s::SoapySDRDevice_getSampleRate(self.device,dir,0).max(0.0).min(u32::MAX as f64) as u32; c.total_bandwidth_hz=current_rate; c.usable_bandwidth_hz=(current_rate as f64*0.9) as u32;
            ...
            let mut sn=0usize; let sp=s::SoapySDRDevice_getSettingInfo(self.device,&mut sn); for i in 0..sn { let a=&*sp.add(i); let key=string(a.key); let mut options=Vec::new(); for j in 0..a.numOptions { options.push(string(*a.options.add(j))); } let kind=match a.type_ { 0=>"bool",1=>"int",2=>"float",_=>"string" }.to_string(); c.settings.push(DeviceSetting{key:key.clone(),name:{let n=string(a.name);if n.is_empty(){key.clone()}else{n}},value:string(s::SoapySDRDevice_readSetting(self.device,CString::new(key).unwrap().as_ptr())),kind,min:a.range.minimum,max:a.range.maximum,step:a.range.step,options}); } if !sp.is_null(){s::SoapySDRArgInfoList_clear(sp,sn);}
...
impl DeviceLayer {
        ...
        if key != "driver=mock" {
            #[cfg(feature="soapysdr")]
            {
                ...
                *self.hardware.lock() = Some(hardware);
                ...
                status.driver = key.split(',').find_map(|part| part.trim().strip_prefix("driver=")).unwrap_or("soapy").to_string();
            ...
            #[cfg(not(feature="soapysdr"))]
            {
                ...
                return Err(anyhow::anyhow!("SoapySDR support is not present in this build"));
            }
        }
        let mut status = self.state.lock();
        status.connected = true;
    ...
    pub fn recover(&self) -> anyhow::Result<()> {
        let key = self.last_key.lock().clone().ok_or_else(|| anyhow::anyhow!("no previous device to recover"))?;
        if key == "driver=mock" { return Ok(()); }
    ...
    pub fn set_frequency(&self, freq: u64) -> anyhow::Result<()> {
        #[cfg(feature="soapysdr")]
        if let Some(hardware) = self.hardware.lock().as_mut() { hardware.set_frequency(freq)?; }
    ...
    pub fn set_sample_rate(&self, rate: u32) -> anyhow::Result<()> {
        if rate == 0 { return Err(anyhow::anyhow!("sample rate must be greater than zero")); }
        #[cfg(feature="soapysdr")]
        if let Some(hardware) = self.hardware.lock().as_mut() { hardware.set_rate(rate)?; }
        self.state.lock().sample_rate = rate;
        Ok(())
    }

    pub fn set_bandwidth(&self, bandwidth: u32) -> anyhow::Result<()> {
        if bandwidth == 0 { return Err(anyhow::anyhow!("bandwidth must be greater than zero")); }
        #[cfg(feature="soapysdr")]
        if let Some(hardware) = self.hardware.lock().as_mut() { hardware.set_bandwidth(bandwidth)?; }
        Ok(())
    }
    ...
    pub fn set_gain(&self, gain: String) -> anyhow::Result<()> {
        ...
        if let Ok(value) = gain.parse::<f64>() {
            if let Some(hardware) = self.hardware.lock().as_mut() { hardware.set_gain(value)?; }
        }
    ...
    pub fn capabilities(&self) -> DeviceCapabilities {
        ...
        if let Some(hardware) = self.hardware.lock().as_ref() {
            ...
            if capabilities.supported_sample_rates_hz.is_empty() {
                capabilities.supported_sample_rates_hz.push(status.sample_rate);
            }
        ...
        DeviceCapabilities {
            ...
            rf_ranges_hz: vec![NumericRange { minimum: 0.0, maximum: 6_000_000_000.0, step: 1.0 }],
            sample_rate_ranges_hz: vec![NumericRange { minimum: 48_000.0, maximum: 20_000_000.0, step: 1.0 }],
            supported_sample_rates_hz: vec![2_000_000, 2_400_000, 5_000_000, 10_000_000],
            bandwidth_ranges_hz: vec![NumericRange { minimum: 12_000.0, maximum: 10_000_000.0, step: 1.0 }],
    ...
    fn identity(&self, status: &DeviceStatus) -> DeviceIdentity {
        let key = self.last_key.lock().clone().unwrap_or_else(|| format!("driver={}", status.driver));
        let serial = key.split(',').find_map(|part| part.trim().strip_prefix("serial=")).map(str::to_owned);
        DeviceIdentity {
            stable_id: serial.as_ref().map(|serial| format!("{}:{serial}", status.driver)).unwrap_or_else(|| key.clone()),
            driver: status.driver.clone(),
            label: status.label.clone(),
            serial,
            connection: if status.driver == "mock" { "virtual".into() } else { "usb_or_network".into() },
            hardware_key: key,
    ...
    pub fn stream_mtu(&self) -> usize { self.capabilities().stream_mtu }
    ...
    fn observe_read(&self, requested: usize, result: anyhow::Result<Vec<Complex<f32>>>) -> anyhow::Result<Vec<Complex<f32>>> {
        self.counters.read_calls.fetch_add(1, Ordering::Relaxed);
        match result {
            Ok(samples) => {
                self.counters.samples_received.fetch_add(samples.len() as u64, Ordering::Relaxed);
                if samples.len() < requested { self.counters.short_reads.fetch_add(1, Ordering::Relaxed); }
                self.counters.consecutive_errors.store(0, Ordering::Relaxed);
                self.counters.last_sample_ms.store(crate::scanner::now_ms().max(0) as u64, Ordering::Relaxed);
    ...
    pub fn read_iq(&self, count: usize) -> anyhow::Result<Vec<Complex<f32>>> {
        ...
        if self.hardware.lock().is_some() {
            let result = self.hardware.lock().as_mut().expect("hardware checked").read(count);
            return self.observe_read(count, result);
        }
        let status = self.status();
        if !status.connected || status.driver != "mock" {
            return self.observe_read(count, Err(anyhow::anyhow!("hardware IQ stream unavailable")));
        }
