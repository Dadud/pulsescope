...
pub struct DeviceStatus {
    ...
    /// Lower edge of hardware tuning range (Hz), refreshed from capabilities.
    #[serde(default = "default_min_freq_hz")]
    pub min_freq_hz: u64,
    /// Upper edge of hardware tuning range (Hz), refreshed from capabilities.
...
impl DeviceLayer {
    ...
    pub fn status(&self) -> DeviceStatus {
        ...
        let (min_hz, max_hz) = self.tuning_range_hz();
        status.min_freq_hz = min_hz;
        status.max_freq_hz = max_hz;
        status
    }

    pub fn tuning_range_hz(&self) -> (u64, u64) {
        let caps = self.capabilities();
        if caps.rf_ranges_hz.is_empty() {
        ...
        if min.is_finite() && max > min {
            (min.max(0.0) as u64, max as u64)
        } else {
            (default_min_freq_hz(), default_max_freq_hz())
        }
    }
